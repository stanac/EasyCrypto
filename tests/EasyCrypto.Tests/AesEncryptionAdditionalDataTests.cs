...
public class AesEncryptionAdditionalDataTests
{
    ...
    public void SerializedEncryptedAdditionalDataCanBeDeserialized()
    {
        ...
        byte[] serialized = (byte[]) additionalDataType.GetMethod("GetBytes", BindingFlags.Instance | BindingFlags.NonPublic)
            .Invoke(additionalDataObj, new object[0]);
        var dataProperty = additionalDataType.GetProperty("Data", BindingFlags.Instance | BindingFlags.Public);
        var additionalDataDeserializerObj = additionalDataType.GetMethod("LoadFromBytes", BindingFlags.Static | BindingFlags.NonPublic)
            .Invoke(null, new object[] {serialized});
    ...
    public void AdditionalDataCanBeRead()
    {
        for (int i = 0; i < NumberOrTestRuns; i++)
        {
            ...
            {
                string password = Guid.NewGuid().ToString();
                string plainText = PasswordGenerator.Default.Generate();
