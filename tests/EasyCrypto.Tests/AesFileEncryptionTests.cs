...
namespace EasyCrypto.Tests
{
    public class AesFileEncryptionTests
    {
        ...
        {
            string pass = PasswordGenerator.Default.Generate();
            ...
            if (async)
            {
                await AesFileEncryption.EncryptWithPasswordAsync(plainTextFile, encryptedFile, pass, true);
                await AesFileEncryption.DecryptWithPasswordAsync(encryptedFile, decryptedFile, pass, true);
            }
            else
            {
                AesFileEncryption.EncryptWithPassword(plainTextFile, encryptedFile, pass, true);
                AesFileEncryption.DecryptWithPassword(encryptedFile, decryptedFile, pass, true);
            }
        ...
        private async Task AssertFileEncryption(int fileLength, bool async)
        {
            ...
            if (async)
            {
                await AesFileEncryption.EncryptAsync(plainTextFile, encryptedFile, key, iv, true);
                await AesFileEncryption.DecryptAsync(encryptedFile, decryptedFile, key, iv, true);
            }
            else
            {
                AesFileEncryption.Encrypt(plainTextFile, encryptedFile, key, iv, true);
                AesFileEncryption.Decrypt(encryptedFile, decryptedFile, key, iv, true);
            }
