...
```
{timeBasedPart, 8 chars}{fixed-part, default is empty}{random parts 4-100 chars, default is 6}
```

IdGenerator can optional fixed part between time based part and random part and can optionally
add hyphens between parts.

How it works:
- Custom base 55 charset (number and English alphabet lower case and upper case excludes chars `I`, `i`,
...
- Fixed part is defined is appended
...
with fixed part and hyphens:         2Qj4AbNM-ABC-z7NHzP
...
// Generates new id
string NewId(DateTime currentTime);
```
...
// make sure when using specific time to use very precise time, including milliseconds
```
